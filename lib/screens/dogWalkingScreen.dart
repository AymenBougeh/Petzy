...
import 'package:pet_user_app/screens/dogWalkingBookingProcessFilterScreen.dart';
...
class _DogWalkingScreenState extends BaseRouteState {
  ...
  Future<void> fetchDogWorkers() async {
    final workers = await _apiUtils.getAllWorker();
    final filteredWorkers =
        workers.where((worker) => worker.role == 'Dog Walking').toList();
    setState(() {
      dogWorkers = filteredWorkers;
    });
  }
                                      ...
                                      GestureDetector(
                                        onTap: () async {
                                          ...
                                          prefs.setInt("workerId",
                                              dogWorkers[index].workerId);
                                          Navigator.of(context).push(
                                              MaterialPageRoute(
                                                  builder: (context) =>
                                                      DogWalkingDetailScreen(
                                                        a: widget.analytics,
                                                        o: widget.observer,
                                                      )));
                                        },
